...
inline OStream& operator<<(OStream& os, Orientation o)
{
...
struct Rectangle
{
    ...
    Rectangle operator*(const Pair<Orientation, Number>& v) const
    {
        auto orientation = v.first;
        auto ratio = v.second;
        ratio = std::clamp(ratio, std::numeric_limits<Number>::min(), One);
...
inline OStream& operator<<(OStream& os, const Rectangle& v)
{
...
inline OStream& operator<<(OStream& os, const Layout& v)
{
...
inline Array<Rectangle> TravelLayout(Layout* CurrentLayout, size_t count)
{
    Array<Rectangle> Result;
...
inline Array<Rectangle> SolveSquarifiedTreemap(const Array<Number>& Datas, const Rectangle& Container)
{
